...
rawcount <- read.csv("counts_anno_ko_neo.csv",row.names = 1)
...
group1=rep(c("KO","NEO"),each=3)
group_list1=factor(group1,levels = c("KO","NEO"))
...
#write.csv(DEG_edgeR1,file="DEG_KO vs NEO.csv")
...
p4.3 <-EnhancedVolcano(data1,
                       ...
                       pCutoff = 0.05,
                       FCcutoff = 0.25,
                       title = "WTvsNEO-down780up478",
                       drawConnectors = TRUE)
p4.3
up_genes <- rownames(DEG_edgeR1[DEG_edgeR1$regulated=="up",])
#write.csv(up_genes,file = "KOvsNEO_DEG_up.csv")
down_genes <- rownames(DEG_edgeR1[DEG_edgeR1$regulated=="down",])
write.csv(down_genes,file = "KOvsNEO_DEG_down.csv")
